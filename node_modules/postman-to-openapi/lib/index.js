  ...
  for (let [i, pjElement] of postmanJson.entries()) {
    ...
    for (let [i, element] of items.entries()) {
      ...
      paths[joinedPath][method.toLowerCase()] = {
        ...
        ...parseParameters(query, header, joinedPath, {}, op),
...
function recuciveJSON(object1, op) {
  ...
  for (const [key, value] of Object.entries(object1)) {
    ...
    if (!Array.isArray(value) && typeof value === 'object' && value !== null) {
      ...
      prop[key] = {}
      prop[key].items = {}
      prop[key].items.properties = recuciveJSON(value[0], newOp);
      prop[key].title = key;
      prop[key].type = "array";
    }else {
      prop[key] = {};
    ...
    if(op.required != null && key in op.required){
      ...
      if(!Array.isArray(value)){
        for (const [key2, _] of Object.entries(value)) {
          if (key2 in op.required[key]){
            if(op.required[key][key2]["$val"] !== undefined)
              required.push(key2)
          }
        }
      }else{
        for (const [key2, _] of Object.entries(value[0])) {
          if (key2 in op.required[key]){
            if(op.required[key][key2]["$val"] !== undefined)
              required.push(key2)
          }
      ...
      if(required.length > 0){
        if(!Array.isArray(value))
          prop[key].required = required
        else{
          prop[key].items.required = required
        }
      }
    }
  }
...
function parseParameters(query = [], header, paths, paramsMeta = {},op) {
  // parse Headers
  let parameters = header.reduce(mapParameters('header',op), [])
  // parse Query
  parameters = query.reduce(mapParameters('query',op), parameters)
  // Path params
...
function mapParameters(type,op) {
  return (parameters, {
    key,
    description,
    value
    ...
    var required = false
    if(op.required !== undefined && op.required.header !== undefined && op.required.header[key] !== undefined){
      required = op.required.header[key]["$val"]
    }
    parameters.push({
      name: key,
      in: type,
...
function parseResponse(response = [], op) {
  ...
  for (let [i, element] of response.entries()) {
    if (element.name.toUpperCase() != "SCHEMA") {
      ...
      if(required.length > 0 )
        status[element.code].content["application/json"].schema.required = required
    }
  }
