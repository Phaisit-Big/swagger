        ...
        content = {
          'application/json': {
            schema: {
              ...
              properties: recuciveJSON("|body",JSON.parse(raw),op)
            }
          }
        }
      } else {
...
function readOption (events = []) {
  ...
  for (let i=0;i < preExe.length;i++){
    if (preExe[i].includes("description") || preExe[i].includes("require")){
...
function parseResponse (response = [],op) {
  ...
  for (let [i, element] of response.entries()){
    ...
    var description = ""
    if(element.originalRequest.body.raw!=""){
      ...
      content:{
          "application/json":{
            schema: {
              type: 'object',
              // example: body,
              properties: recuciveJSON("|body",body,op)
            }
          }
      }
    }
